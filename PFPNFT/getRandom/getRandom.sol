...
contract GetRandomContract is Manager, math{
    ...
    function getTRandom(uint _mintIndex, bool _isWithBadge) public view returns(
		...
		uint genderRate = _random(block.number+_mintIndex, 0, 9);
		uint _GetGander = 0;
		if(genderRate > 4){
			_GetGander = 1;
		}
		...
		uint raceRate = _random(block.number+_mintIndex, raceStart, 10000);
		uint _GetRace = 0;
		if(raceRate >= 8000 && raceRate < 9750){
			_GetRace = 1;
		}else if(raceRate >= 9750 && raceRate < 9950){
			_GetRace = 2;
		}else if(raceRate >= 9950 && raceRate < 9990){
			_GetRace = 3;
		}else if(raceRate >= 9990 && raceRate < 9998){
			_GetRace = 4;
		}else if(raceRate >= 9998){
			_GetRace = 5;
		}

		uint raceMask = _random(block.number+_mintIndex, 0, 100);
		uint _GetMask = 0;
		if(raceMask >= 70){
			_GetMask = _random(block.number+_mintIndex, 0, 8);
		}
		
		uint raceGlasses = _random(block.number+_mintIndex, 0, 100);
		uint _GetGlasses = 0;
		if(raceGlasses >= 70){
			_GetGlasses = _random(block.number+_mintIndex, 0, 8);
		}
		
		uint raceAC = _random(block.number+_mintIndex, 0, 100);
		uint _GetAC = 0;
		if(raceAC >= 70){
			_GetAC = _random(block.number+_mintIndex, 0, 13);
		}

		uint[] memory RandomRate = new uint[](10);
		RandomRate[0] = _GetGander;
		RandomRate[1] = _random(block.number+_mintIndex, 0, 7); //background
		RandomRate[2] = _GetRace;
		RandomRate[3] = _random(block.number+_mintIndex, 0, 8); //hairStyle
		RandomRate[4] = _random(block.number+_mintIndex, 0, 8); //eyes
		RandomRate[5] = _random(block.number+_mintIndex, 0, 7); //mouth
		RandomRate[6] = _random(block.number+_mintIndex, 0, 7); //clothes
