...
contract AirDrop88888MTC is Manager, math{
    ...
    mapping (address => uint256) public totalMTCGive;
    ...
    function AddMTCGiveAmounts(uint[] memory _MTCGiveAmounts) public onlyManager returns (bool) {
        uint _addressAmount = _MTCGiveAmounts.length;
        for (uint i = 0; i < _addressAmount; i++){
    ...
    function calculateAirDropMTC(uint _DropMin, uint _DropMax) public onlyManager{
        for (uint i = _DropMin; i < _DropMax; i++){
			totalMTCGive[participantList[i]] += MTCGiveAmounts[i];
        }
    }
    ...
    function AirDropMTC(uint _Sort) internal{
		address _SortAddr = getWhitelist(_Sort);
		require(AirDrop(_MTCAddr).transfer(_SortAddr, totalMTCGive[_SortAddr]));
    }
